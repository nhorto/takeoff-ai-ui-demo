...
import { buttonClass, cx, panelEyebrowClass } from "@/components/ui/uiStyles";
    ...
    <div className="space-y-4">
      ...
      <div>
        ...
        {railTemplates.length === 0 && (
          <span className="ml-3 text-xs text-white/52">
                  ...
                  <div className="mt-2 space-y-1.5">
                    {list.map((t) => (
                      <button
                        ...
                        className="w-full rounded-md border border-white/10 bg-white/[0.03] px-3 py-2.5 text-left text-sm text-white/84 transition hover:border-cyan-300/40 hover:bg-cyan-300/[0.08]"
                      >
                        {t.name}
                      </button>
                    ))}
...
function RailAssignmentCard({
  ...
  return (
    <div className="rounded-xl border border-white/10 bg-white/[0.03] p-4 shadow-[inset_0_1px_0_rgba(255,255,255,0.03)]">
      <div className="mb-3 flex items-center justify-between gap-3">
        ...
        <button
          ...
          className={cx(buttonClass.destructive, "shrink-0 px-3 py-1.5 text-xs")}
        >
          Remove
        </button>
      </div>
